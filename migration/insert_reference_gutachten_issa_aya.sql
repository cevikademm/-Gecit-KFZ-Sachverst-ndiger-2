-- ════════════════════════════════════════════════════════════════════════
-- Referans Gutachten — Issa Aya (Smart forfour, AC AI 88)
-- ════════════════════════════════════════════════════════════════════════
-- Aktenzeichen: GA-HS-2026-04-052
-- Kullanıcı tarafından PDF analizi için referans olarak verildi.
-- AI Agent ile dolduruldu işaretiyle Ekspertiz tab'ında görünür.
--
-- Çalıştırma: Supabase Dashboard → SQL Editor → bu dosyayı yapıştır → Run
-- Idempotent: zaten ekliyse tekrar eklemez.
-- ════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_customer_id IS NULL THEN
    ...
    VALUES (
      'Issa Aya', 'bireysel',
      'Robert Koch Str. 6', '52477', 'Alsdorf',
      now()
    )
  ...
  IF v_vehicle_id IS NULL THEN
    INSERT INTO vehicles (
      owner_id, plate, brand, model, year,
      chassis, color, fuel, engine_cc, created_at
    )
    VALUES (
      v_customer_id, 'AC AI 88',
      'Smart', 'forfour (11.2014->2022)', 2016,
      'WME4530441Y068496', 'Grau Silber', 'Diesel', 898,
      now()
    )
    RETURNING id INTO v_vehicle_id;
    RAISE NOTICE 'Yeni araç oluşturuldu: % (AC AI 88)', v_vehicle_id;
  ELSE
    -- Mevcut araç başka bir müşteriye atanmışsa Issa'ya devret (defansif)
    UPDATE vehicles SET owner_id = v_customer_id WHERE id = v_vehicle_id AND owner_id IS NULL;
    RAISE NOTICE 'Mevcut araç kullanıldı: %', v_vehicle_id;
  END IF;

  -- ─── 3. Appraisal: Aktenzeichen GA-HS-2026-04-052 ──────────────────
  ...
  INSERT INTO appraisals (
    ...
    notes, draft_data, created_by, source,
    ...
    $JSON$
{
  ...
  "vehicle": {
    ...
    "powerPs": "90",
    "displacement": "898",
    "engineConfig": "3-Zylinder / Reihenmotor",
    "cylinders": "3",
    "transmission": "5-Gang manuell",
    "yearOfManufacture": "2016",
    "firstRegistration": "2016-03-23",
    "lastRegistration": "2025-05-28",
  ...
  "condition": {
    ...
    "emissionGroup": 4,
    "emissionNorm": "EU6",
    "paintColor": "Grau Silber Metallic",
    "paintType": "2-Schicht",
    "paintCode": "ER2"
  },
  "damages": {
    "areas": {
      ...
      "frontCenter": true,
      "frontRight": false,
      "fenderFrontLeft": false,
      "hood": false,
      "fenderFrontRight": false,
      "doorDriver": false,
      "windshield": false,
      "doorFrontPassenger": false,
      "doorBackPassengerLeft": false,
      "roof": false,
      "doorBackPassengerRight": false,
      "fenderRearLeft": false,
      "fenderRearRight": false,
      "rearLeft": false,
      "rearWindow": false,
      "rearCenter": false,
      "rearRight": true
  ...
  "calculation": {
    "provider": "dat",
    "repairCostNet": "3129.04",
    "repairCostGross": "3723.56",
    "vatRate": 19,
    "laborHours": "10.55",
    "laborCostPerHour": "178.50",
    "paintCostNet": "2354.96",
    "sparePartsNet": "328.26",
    "sparePartsSurcharge": 20,
    "smallParts": 2,
    "devaluation": "0",
    "replacementValue": "8450",
    "residualValue": "",
    "repairDuration": "3",
    "isTotalLoss": false,
